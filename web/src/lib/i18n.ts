export type Locale = 'zh' | 'en';

export const DEFAULT_LOCALE: Locale = 'zh';

export const messages = {
  zh: {
    app: {
      name: 'SingVid',
      tagline: '创建同步歌词视频',
      newProject: '新建项目',
      startCreating: '开始创建',
      yourProjects: '你的项目',
      total: '共 {count} 个',
      noProjects: '还没有项目',
      createFirst: '上传音频，创建第一个歌词视频',
      createProject: '创建项目',
      heroTitle1: '歌词视频',
      heroTitle2: '几分钟搞定',
      heroDesc: '上传音频，自动对齐歌词，微调时间线，导出精美歌词视频——无需编辑经验。',
      uploadCreate: '上传并创建',
      templates: '模板',
      notesTemplate: '笔记模板',
      notesTemplateAlt: '笔记模板示例截图',
      voiceMemoTemplate: '语音备忘录模板',
      voiceMemoTemplateAlt: '语音备忘录模板示例截图',
      deleteProject: '删除',
      deleteProjectConfirm: '确定删除项目"{title}"吗？此操作不可撤销。',
      deleteProjectFailed: '删除项目失败，请重试',
    },
    create: {
      title: '上传音频创建项目',
      projectTitle: '歌曲标题',
      projectPlaceholder: '我的歌曲',
      brand: '品牌',
      brandPlaceholder: '品牌名称',
      template: '模板',
      singer: '歌手 (副标题)',
      singerPlaceholder: '歌手名称',
      manualLyrics: '歌词 (可选)',
      manualLyricsPlaceholder: '在此粘贴歌词，每行一句…\n在下一页面可调整时间线。',
      audioFile: '音频文件 (MP3, WAV, M4A)',
      uploadCreate: '上传并创建',
      uploading: '上传中...',
      cancel: '取消',
      uploadFailed: '上传失败，请重试',
    },
    editor: {
      ready: '就绪',
      noLyrics: '无歌词',
      rendering: '渲染中...',
      renderMP4: '渲染 MP4',
      workflow: '工作流',
      generate: '生成',
      transcribe: '识别歌词',
      assisted: '辅助对齐',
      align: '精确对齐',
      preview: '预览',
      lyrics: '歌词',
      timeline: '时间线',
      audio: '音频',
      space: '空格',
      noAudio: '未上传音频',
      projectNotFound: '项目未找到',
      assistedResult: '辅助对齐：{matched}/{total} 匹配，{fallback} 补位',
      downloadRendered: '下载视频',
      previewRendered: '预览视频',
      renderReady: '视频已渲染完成',
    },
    lyrics: {
      title: '歌词',
      save: '保存歌词',
      saving: '保存中...',
      unsaved: '未保存',
      placeholder: '在这里粘贴歌词，每行一句...',
      emptyTip: '粘贴歌词后，每一行会成为独立歌词行。',
      lines: '{count} 行',
      source: '来源：{source}',
    },
    timeline: {
      autoSuffix: '自动补尾',
      save: '保存',
      start: '开始',
      end: '结束',
      tap: '打点',
      noLines: '还没有歌词行。',
      currentTime: '{time}',
      tapHint: '点击设置起点',
      nudgeHint: '左右 0.5 秒微调',
    },
    jobs: {
      transcribe: '歌词识别',
      render: '视频渲染',
      align: '精确对齐',
      queued: '排队中...',
      processing: '处理中...',
      done: '完成',
      failed: '失败：{error}',
      complete: '完成',
    },
    common: {
      back: '返回',
      space: '空格',
      source: '来源',
      lines: '行',
      empty: '(空)',
    },
    ai: {
      settings: 'AI 设置',
      settingsTitle: '配置 OpenAI-compatible LLM',
      title: '全局 AI 配置',
      description: '配置 OpenAI-compatible LLM，用于识别后按歌曲名修正歌词。未配置时不会使用 AI，本地识别和编辑流程照常工作。配置仅保存在当前浏览器。',
      enabled: '启用 AI 纠错',
      baseUrl: 'Base URL',
      apiKey: 'API Key',
      model: '模型',
      temperature: 'Temperature',
      showKey: '显示',
      hideKey: '隐藏',
      save: '保存配置',
      cancel: '取消',
      testConnection: '检测连接',
      testingConnection: '检测中...',
      connectionOk: '连接可用',
      connectionFailed: '连接不可用',
      noLines: '没有可纠错的歌词行',
      correcting: 'AI 正在自动纠错识别歌词...',
      corrected: 'AI 已完成歌词纠错',
      correctFailed: 'AI 纠错失败',
      validationRequired: '启用后必须填写 Base URL、API Key 和模型',
      validationTemperature: 'Temperature 必须在 0 到 2 之间',
      fetchModels: '拉取模型列表',
      loadingModels: '拉取中…',
      noModels: '未拉取到模型，请手动填写',
      modelCustom: '自定义',
      useCustomInput: '使用自定义输入',
      backToList: '返回下拉选择',
    },
  },
  en: {
    app: {
      name: 'SingVid',
      tagline: 'Create synced lyric videos',
      newProject: 'New Project',
      startCreating: 'Start Creating',
      yourProjects: 'Your Projects',
      total: '{count} total',
      noProjects: 'No projects yet',
      createFirst: 'Upload audio to create your first lyric video',
      createProject: 'Create Project',
      heroTitle1: 'Lyric videos',
      heroTitle2: 'in minutes',
      heroDesc: 'Upload your audio, auto-sync lyrics, tweak the timeline, and render stunning lyric videos — no video editing experience needed.',
      uploadCreate: 'Upload & Create',
      templates: 'Templates',
      notesTemplate: 'Notes template',
      notesTemplateAlt: 'Notes template preview image',
      voiceMemoTemplate: 'Voice memo template',
      voiceMemoTemplateAlt: 'Voice memo template preview image',
      deleteProject: 'Delete',
      deleteProjectConfirm: 'Delete project "{title}"? This cannot be undone.',
      deleteProjectFailed: 'Failed to delete project, please try again',
    },
    create: {
      title: 'Upload audio to create a project',
      projectTitle: 'Song title',
      projectPlaceholder: 'My Song',
      brand: 'Brand',
      brandPlaceholder: 'Brand name',
      template: 'Template',
      singer: 'Singer (subtitle)',
      singerPlaceholder: 'Singer name',
      manualLyrics: 'Lyrics (optional)',
      manualLyricsPlaceholder: 'Paste lyrics here, one line per phrase…\nYou can adjust timings on the next page.',
      audioFile: 'Audio file (MP3, WAV, M4A)',
      uploadCreate: 'Upload & Create',
      uploading: 'Uploading...',
      cancel: 'Cancel',
      uploadFailed: 'Upload failed, please try again',
    },
    editor: {
      ready: 'Ready',
      noLyrics: 'No lyrics',
      rendering: 'Rendering...',
      renderMP4: 'Render MP4',
      workflow: 'Workflow',
      generate: 'Generate',
      transcribe: 'Transcribe',
      assisted: 'Assisted',
      align: 'Align',
      preview: 'Preview',
      lyrics: 'Lyrics',
      timeline: 'Timeline',
      audio: 'Audio',
      space: 'Space',
      noAudio: 'No audio uploaded',
      projectNotFound: 'Project not found',
      assistedResult: 'Assisted: {matched}/{total} matched, {fallback} filled',
      previewRendered: 'Preview Video',
      renderReady: 'Video rendered',
    },
    lyrics: {
      title: 'Lyrics',
      save: 'Save Lyrics',
      saving: 'Saving...',
      unsaved: 'Unsaved',
      placeholder: 'Paste your lyrics here, one line per phrase...',
      emptyTip: 'Paste lyrics, each line becomes a separate lyric row.',
      lines: '{count} lines',
      source: 'Source: {source}',
    },
    timeline: {
      autoSuffix: 'Auto-suffix',
      save: 'Save',
      start: 'Start',
      end: 'End',
      tap: 'Tap',
      noLines: 'No lines yet.',
      currentTime: '{time}',
      tapHint: 'Tap to set start',
      nudgeHint: '\u25C0\u25B6 0.5s',
    },
    jobs: {
      transcribe: 'Transcribe',
      render: 'Render',
      align: 'Align',
      queued: 'Queued...',
      processing: 'Processing...',
      done: 'Done',
      failed: 'Failed: {error}',
      complete: 'Complete',
    },
    common: {
      back: 'Back',
      space: 'Space',
      source: 'Source',
      lines: 'lines',
      empty: '(empty)',
    },
    ai: {
      settings: 'AI Settings',
      settingsTitle: 'Configure an OpenAI-compatible LLM',
      title: 'Global AI Settings',
      description: 'Configure an OpenAI-compatible LLM to correct recognized lyrics by song title. When not configured, AI is not used and the local workflow keeps working. Settings are stored only in this browser.',
      enabled: 'Enable AI correction',
      baseUrl: 'Base URL',
      apiKey: 'API Key',
      model: 'Model',
      temperature: 'Temperature',
      showKey: 'Show',
      hideKey: 'Hide',
      save: 'Save settings',
      cancel: 'Cancel',
      testConnection: 'Test connection',
      testingConnection: 'Testing...',
      connectionOk: 'Connection available',
      connectionFailed: 'Connection unavailable',
      noLines: 'No lyric lines to correct',
      correcting: 'AI is automatically correcting recognized lyrics...',
      corrected: 'AI lyric correction completed',
      correctFailed: 'AI correction failed',
      validationRequired: 'Base URL, API key and model are required when enabled',
      validationTemperature: 'Temperature must be between 0 and 2',
      fetchModels: 'Fetch models',
      loadingModels: 'Loading…',
      noModels: 'No models returned, please type one manually',
      modelCustom: 'Custom',
      useCustomInput: 'Use custom input',
      backToList: 'Back to list',
    },
  },
} as const;

type Messages = typeof messages;
type Section = keyof Messages['zh'];

export function t(key: string, values: Record<string, string | number> = {}): string {
  const parts = key.split('.');
  const locale = getCurrentLocale();
  let obj: any = messages[locale];
  for (const part of parts) {
    if (obj == null) return key;
    obj = obj[part];
  }
  if (typeof obj !== 'string') return key;
  return obj.replace(/\{(\w+)\}/g, (_, k) => String(values[k] ?? `{${k}}`));
}

function getCurrentLocale(): Locale {
  if (typeof window === 'undefined') return DEFAULT_LOCALE;
  const stored = localStorage.getItem('locale');
  if (stored === 'zh' || stored === 'en') return stored;
  return DEFAULT_LOCALE;
}
